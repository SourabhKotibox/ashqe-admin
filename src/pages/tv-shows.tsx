...
import {
  useGetTVShows,
  useDeleteTVShow,
  useToggleTVShowFeatured,
  useToggleTVShowTrending,
  useUpdateTVShowStatus,
  useGetLanguagesList,
...
export default function TvShowsPage() {
  ...
  const { data: tvShowsData, isLoading } = useGetTVShows({
    page: 1,
    limit: 100,
  ...
  const toggleFeaturedMutation = useToggleTVShowFeatured();
  const toggleTrendingMutation = useToggleTVShowTrending();
  const updateStatusMutation = useUpdateTVShowStatus();

  const filtered = tvShows.filter((m) => {
    const matchStatus =
      statusFilter === "all" ||
  ...
  const handleToggleStatus = (id: string, currentStatus: string) => {
    ...
    updateStatusMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: `Web Series ${newStatus === 'published' ? 'published' : 'unpublished'} successfully!` });
        },
      }
    );
  };
  ...
  const handleDelete = () => {
    ...
    deleteMutation.mutate(confirmDelete.id, {
      onSuccess: () => {
        toast({ title: "Web Series deleted successfully!" });
  ...
  const executeBulkAction = async () => {
    try {
      if (bulkAction === "activate") {
        ...
        toast({ title: `${selectedIds.length} tvShow(s) published.` });
        ...
        toast({ title: `${selectedIds.length} tvShow(s) set to draft.` });
      }
      setSelectedIds([]);
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      toast({ title: `${selectedIds.length} tvShow(s) deleted successfully.` });
      setSelectedIds([]);
      queryClient.invalidateQueries({ queryKey: ["tv-shows"] });
      setBulkAction("action");
    } catch {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <span>Dashboard</span>
        <span>/</span>
        <span className="text-foreground font-medium">Web Series</span>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border bg-card hover:bg-card">
                ...
                <TableHead className="text-foreground/70 font-semibold text-xs uppercase tracking-wide min-w-[200px]">Web Series</TableHead>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                ...
                filtered.map((tvShow) => {
                  const planBadge = planBadgeFor(tvShow.planRequired);
                  const statusBadge = STATUS_BADGE[tvShow.status] || STATUS_BADGE.draft;
                  return (
                    <TableRow key={tvShow.id} className="border-border hover:bg-muted/30">
                      <TableCell>
                        <Checkbox
                          checked={selectedIds.includes(tvShow.id)}
                          onCheckedChange={() => toggleOne(tvShow.id)}
                          className="border-border data-[state=checked]:bg-primary data-[state=checked]:border-red-600"
                        />
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <div className="h-16 w-11 rounded-lg overflow-hidden border border-border bg-muted shrink-0 flex items-center justify-center">
                            {tvShow.thumbnail ? (
                              <img src={getImageUrl(tvShow.thumbnail)} alt={tvShow.title} className="h-full w-full object-contain" />
                            ) : (
                              <ImageIcon className="h-5 w-5 text-muted-foreground/80" />
                            )}
                          </div>
                          <div>
                            <p className="text-foreground font-medium text-sm">{tvShow.title}</p>
                            ...
                            {tvShow.releaseDate && (
                              <p className="text-xs text-muted-foreground/80 mt-0.5">{new Date(tvShow.releaseDate).toLocaleDateString()}</p>
                            )}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="text-foreground/70 text-sm">{tvShow.likes}</TableCell>
                      <TableCell className="text-foreground/70 text-sm">{tvShow.views}</TableCell>
                      <TableCell>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium ${planBadge.className}`}>
                          {planBadge.label}
                        </span>
                      </TableCell>
                      <TableCell className="text-foreground/70 text-sm">{tvShow.languages[0]?.name || '-'}</TableCell>
                      <TableCell>
                        <button
                          onClick={() => handleToggleStatus(tvShow.id, tvShow.status)}
                      ...
                      <TableCell>
                        <Switch
                          checked={tvShow.featured}
                          onCheckedChange={() => handleToggleFeatured(tvShow.id)}
                          className="data-[state=checked]:bg-primary"
                        />
                      </TableCell>
                      <TableCell>
                        <Switch
                          checked={tvShow.trending}
                          onCheckedChange={() => handleToggleTrending(tvShow.id)}
                          className="data-[state=checked]:bg-primary"
                        />
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => setLocation(`/tv-shows/${tvShow.id}/edit`)}
                          ...
                          <button
                            onClick={() => setConfirmDelete(tvShow)}
      ...
      <AlertDialog open={!!confirmDelete} onOpenChange={() => setConfirmDelete(null)}>
        <AlertDialogContent className="bg-card border-border text-foreground">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Web Series</AlertDialogTitle>
      ...
      <AlertDialog open={bulkConfirmOpen} onOpenChange={setBulkConfirmOpen}>
        <AlertDialogContent className="bg-card border-border text-foreground">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete {selectedIds.length} Web Series(s)</AlertDialogTitle>
            <AlertDialogDescription className="text-foreground/70">
              Are you sure you want to delete {selectedIds.length} selected tvShow(s)?
