...
export default function EpisodeForm() {
  ...
  // API
  const { data: showsData, isLoading: loadingShows } = useGetTVShows({ limit: 200 });
  const { data: seasonsData } = useGetSeasonList(tvShowId ? { tvShowId: tvShowId } : {});
  ...
  const handleVideoSelect = (media: any) => {
    ...
    if (media.isHls && media.hlsMasterPlaylistUrl) {
      ...
      if (media.duration) {
        ...
        const selectedShow = tvShows.find((s) => getId(s) === tvShowId);
  ...
  useEffect(() => {
    if (isEdit && existingEpisode) {
      ...
      setTvShowId(e.tvShowId?._id || e.tvShowId || "");
  ...
  const handleSave = async () => {
    if (!tvShowId) {
      toast({ title: "Please select a Web Series", variant: "destructive" });
      return;
    }
    if (!name.trim()) {
        ...
        {activeTab === "Episode Details" && (
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-1.5">
                <Label className="text-foreground text-sm font-medium">
                  Web Series <span className="text-primary">*</span>
                </Label>
                <Select value={tvShowId} onValueChange={setTvShowId} disabled={loadingShows}>
                  <SelectTrigger className="bg-muted border-border text-foreground h-10 rounded-lg text-sm">
                    <SelectValue placeholder={loadingShows ? "Loading…" : "Select Web Series"} />
                  </SelectTrigger>
