...
export default function TVShowDetailPage() {
  ...
  const title = show?.title || "Web Series";
  ...
  return (
    <div className="min-h-screen bg-[#0c0c14] text-foreground">
      <PublicHeader
        activeTab={activeTab}
      ...
      <div className="relative w-full" style={{ height: "70vh", minHeight: 440 }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[#0c0c14] via-[#0c0c14]/60 to-transparent" />
        <div className="absolute bottom-0 left-0 right-0 h-[60%] bg-gradient-to-t from-[#0c0c14] to-transparent" />
        <div className="absolute top-0 left-0 right-0 h-[20%] bg-gradient-to-b from-[#0c0c14]/70 to-transparent" />
        ...
        <div className="absolute bottom-10 left-0 px-6 sm:px-10 lg:px-14 max-w-2xl">
          ...
          <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black text-foreground leading-tight mb-3 tracking-tight drop-shadow-lg">{title}</h1>
          ...
          <p className="text-foreground/80 text-sm sm:text-[15px] leading-relaxed mb-6 max-w-2xl line-clamp-3">{description}</p>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {!isSubscribed && (
              <button
                ...
                className="flex items-center gap-2.5 px-6 py-3.5 bg-primary hover:bg-primary/90 text-white font-bold rounded-lg text-sm tracking-wide transition-all active:scale-95 shadow-lg"
              >
      ...
      <div className="px-6 sm:px-10 lg:px-14 pb-20 mt-6">
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-foreground font-black text-lg sm:text-xl tracking-tight">Episodes</h2>
              ...
              <div className="flex items-center gap-2">
                {uniqueSeasons.map((s) => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-bold border transition-all ${selectedSeason === s ? "bg-primary border-primary text-white" : "bg-transparent border-zinc-700 text-foreground/80 hover:border-zinc-500 hover:text-foreground"}`}
                  >
                    S{s}
                  </button>
                ))}
                ...
                return (
                  ...
                  >
                    <div className="relative w-28 flex-shrink-0 rounded-lg overflow-hidden bg-zinc-800" style={{ aspectRatio: "16/9" }}>
                      ...
                      <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                        {isLocked ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-primary/80 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all scale-90 group-hover:scale-100">
                    ...
                    <div className="flex-1 min-w-0 py-1">
                      ...
                      <p className="text-foreground text-sm font-semibold leading-tight truncate group-hover:text-primary transition-colors">{ep.title || `Episode ${epNum}`}</p>
                      {ep.description && (
        ...
        <div className="mb-12 max-w-3xl">
          <h2 className="text-foreground font-black text-lg sm:text-xl tracking-tight mb-4">About</h2>
        ...
        {cast.length > 0 && (
          <div className="mb-12">
            <h2 className="text-foreground font-black text-lg sm:text-xl tracking-tight mb-4">Cast</h2>
            <div className="flex gap-4 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
              {cast.map((c: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center text-center w-24 sm:w-28 flex-shrink-0 group">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden border border-zinc-800 bg-zinc-900 flex-shrink-0 group-hover:border-primary transition-all duration-300 shadow-md">
                      {actorImg ? (
                        <img src={actorImg} alt={actorName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                      ) : (
                    ...
                    <h4 className="text-foreground font-bold text-xs sm:text-sm mt-3 line-clamp-1 group-hover:text-foreground transition-colors">{actorName}</h4>
                    {c.character && <p className="text-foreground/80 text-[10px] sm:text-xs mt-0.5 line-clamp-1 font-semibold">{c.character}</p>}
                  </div>
                );
              })}
            </div>
