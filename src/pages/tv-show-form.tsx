...
import {
  useGetDirectors, useGetActors, useCreateTVShow, useUpdateTVShow,
  useGetGenres, useGetLanguagesList, useGetTVShowById, useGetCategoriesList,
...
/** Parse "Web Series.Name.2024.1080p.WEB-DL" → title + year */
...
export default function TvShowForm() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>("Web Series Details");
  ...
  const { data: sectionsData } = useGetSections({ contentType: "tvShow", activeOnly: true });
  ...
  const { data: tvShowData } = useGetTVShowById(isEdit ? id! : "");
  const tvShow = (tvShowData as any)?.data;
  ...
  const applyVideoAutoFill = (media: any, _opts?: { onlyEmpty?: boolean }) => {
    ...
    if (nextTitle) {
      ...
      setMetaDescription((prev) => prev || `Watch ${nextTitle} on Ashqe — 18+ tvShows streaming.`);
      setTags((prev) => {
  ...
  const handleVideoSelect = (media: any) => {
    ...
    if (hasPath) {
      toast({
        ...
        description: "Click Update Web Series anytime. HLS qualities will fill in when processing finishes.",
      });
    }
  ...
  useEffect(() => {
    if (!isEdit || !tvShow) return;

    setTitle(tvShow.title || "");
    setOriginalTitle(tvShow.originalTitle || "");
    setDescription(tvShow.description || "");
    setShortDescription(tvShow.shortDescription || "");
    setPlanRequired(tvShow.planRequired || "free");
    setStatus(tvShow.status || "draft");

    if (tvShow.thumbnail)
      setThumbnail({ filePath: tvShow.thumbnail, preview: getImageUrl(tvShow.thumbnail) });
    if (tvShow.posterImage)
      setPoster({ filePath: tvShow.posterImage, preview: getImageUrl(tvShow.posterImage) });
    if (tvShow.bannerImage)
      setBanner({ filePath: tvShow.bannerImage, preview: getImageUrl(tvShow.bannerImage) });
    if (tvShow.seoImage)
      setSeoImage({ filePath: tvShow.seoImage, preview: getImageUrl(tvShow.seoImage) });

    if (tvShow.trailerUrl) {
      if (tvShow.trailerUrl.startsWith("http://") || tvShow.trailerUrl.startsWith("https://")) {
        setTrailerUrlType("url");
        setTrailerUrl(tvShow.trailerUrl);
      } else {
        setTrailerUrlType("local");
        setTrailerFilePath(tvShow.trailerUrl);
      }
    }

    setYear(tvShow.year ? String(tvShow.year) : "");
    setTotalSeasons(tvShow.totalSeasons ? String(tvShow.totalSeasons) : "");
    setRating(tvShow.rating || "");
    setImdbRating(tvShow.imdbRating != null ? String(tvShow.imdbRating) : "");
    setDuration(tvShow.duration ? secsToDuration(tvShow.duration) : "");
    setReleaseDate(
      tvShow.releaseDate ? new Date(tvShow.releaseDate).toISOString().split("T")[0] : ""
    );
    setCountry(tvShow.country || "");
    setProducer(tvShow.producer || "");
    setStudio(tvShow.studio || "");
    setAgeRating(tvShow.ageRating != null ? String(tvShow.ageRating) : "0");
    setDownloadAllowed(tvShow.downloadAllowed !== false);
    setFeatured(!!tvShow.featured);
    setTrending(!!tvShow.trending);
    setIsNewContent(tvShow.isNewContent !== false);
    setIsExclusive(!!tvShow.isExclusive);
    setMaturityContent(Array.isArray(tvShow.maturityContent) ? tvShow.maturityContent : []);
    setTags(Array.isArray(tvShow.tags) ? tvShow.tags : []);

    if (Array.isArray(tvShow.genres))
      setSelectedGenres(tvShow.genres.map(getId).filter(Boolean));
    if (Array.isArray(tvShow.categories))
      setSelectedCategories(tvShow.categories.map(getId).filter(Boolean));
    if (Array.isArray(tvShow.languages))
      setSelectedLanguages(tvShow.languages.map(getId).filter(Boolean));
    if (Array.isArray(tvShow.audioLanguages))
      setSelectedAudioLanguages(tvShow.audioLanguages.map(getId).filter(Boolean));
    if (Array.isArray(tvShow.subtitleLanguages))
      setSelectedSubtitleLanguages(tvShow.subtitleLanguages.map(getId).filter(Boolean));
    if (Array.isArray(tvShow.sections))
      setSelectedSections(tvShow.sections.map(getId).filter(Boolean));

    if (Array.isArray(tvShow.cast)) {
      setCastItems(
        tvShow.cast.map((c: any, i: number) => ({
          id: String(i),
    ...
    if (Array.isArray(tvShow.crew)) {
      setCrewItems(
        tvShow.crew.map((c: any, i: number) => ({
          id: String(i),
    ...
    if (tvShow.hlsUrl) {
      const isHttp = tvShow.hlsUrl.startsWith("http://") || tvShow.hlsUrl.startsWith("https://");
      if (tvShow.hlsUrl.endsWith(".m3u8") && isHttp) {
        setVideoUploadType("hls");
        setVideoUrl(tvShow.hlsUrl);
      } else if (isHttp) {
        setVideoUploadType("url");
        setVideoUrl(tvShow.hlsUrl);
      } else {
        setVideoUploadType("local");
        setVideoFilePath(tvShow.hlsUrl);
      }
    }
    if (Array.isArray(tvShow.videoQualities) && tvShow.videoQualities.length > 0) {
      setQualityEnabled(true);
      setQualityRows(
        tvShow.videoQualities.map((q: any, i: number) => {
    ...
    if (Array.isArray(tvShow.subtitles) && tvShow.subtitles.length > 0) {
      setSubtitleRows(
        tvShow.subtitles.map((s: any, i: number) => ({
          id: String(i),
    ...
    setSlug(tvShow.slug || "");
    setMetaTitle(tvShow.metaTitle || "");
    setMetaDescription(tvShow.metaDescription || "");
  }, [isEdit, tvShow]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await updateTVShowMutation.mutateAsync({ id: id!, data: payload });
      } else {
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <button onClick={() => setLocation("/tvShows")} className="hover:text-foreground transition-colors">
          Dashboard
        </button>
        <span>/</span>
        <button onClick={() => setLocation("/tvShows")} className="hover:text-foreground transition-colors">
        ...
        <span>/</span>
        <span className="text-foreground font-medium">{isEdit ? "Edit Web Series" : "New Web Series"}</span>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card">
        ...
        {/* =========== MOVIE DETAILS =========== */}
        {activeTab === "Web Series Details" && (
          <div className="p-6 space-y-6">
            <SectionHeading title="About Web Series" />
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                placeholder="Full tvShow description..."
                value={description}
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground pt-1">Select the categories where this tvShow should be manually displayed.</p>
            </div>
          </div>
        )}
      ...
      <div className="flex justify-end">
        ...
        >
          {isSaving ? "Saving..." : isEdit ? "Update Web Series" : "Create Web Series"}
        </Button>
      </div>

      {/* Media Pickers */}
      <MediaPicker open={thumbnailPickerOpen} onClose={() => setThumbnailPickerOpen(false)}
        onSelect={(m) => setThumbnail({ filePath: m.filePath, preview: m.url })} source="tvShow" accept="image/*" />
      <MediaPicker open={posterPickerOpen} onClose={() => setPosterPickerOpen(false)}
        onSelect={(m) => setPoster({ filePath: m.filePath, preview: m.url })} source="tvShow" accept="image/*" />
      <MediaPicker open={bannerPickerOpen} onClose={() => setBannerPickerOpen(false)}
        onSelect={(m) => setBanner({ filePath: m.filePath, preview: m.url })} source="tvShow" accept="image/*" />
      <MediaPicker open={trailerPickerOpen} onClose={() => setTrailerPickerOpen(false)}
        onSelect={(m) => setTrailerFilePath(m.filePath)} source="tvShow" accept="video/*" />
      <MediaPicker open={videoPickerOpen} onClose={() => setVideoPickerOpen(false)}
        onSelect={handleVideoSelect} source="tvShow" accept="video/*" />
      <MediaPicker
      ...
      <MediaPicker
        ...
        source="tvShow" accept=".srt,.vtt,.ass,.ssa"
      />
      <MediaPicker open={seoImagePickerOpen} onClose={() => setSeoImagePickerOpen(false)}
        onSelect={(m) => setSeoImage({ filePath: m.filePath, preview: m.url })} source="tvShow" accept="image/*" />
    </div>
  );
}
