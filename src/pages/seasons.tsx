...
export default function SeasonsPage() {
  ...
  // Delete a season = delete all episodes in that tvShowId + season number
  const handleDeleteSeasonEpisodes = async () => {
    ...
    try {
      ...
      const toDelete = allEpisodes.filter(
        (ep: any) =>
          ep.tvShowId?._id?.toString() === confirmDelete.tvShowId?.toString() &&
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative">
          ...
          <Input
            placeholder="Search Web Seriess..."
            value={searchQuery}
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border bg-card hover:bg-card">
                ...
                <TableHead className="text-foreground/70 font-semibold text-xs uppercase tracking-wide">Web Series</TableHead>
              ...
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-foreground/65 py-14">
                    {searchQuery ? "No seasons match your search" : "No seasons yet. Add episodes to a Web Series to create seasons."}
                  </TableCell>
                </TableRow>
              ) : (
                filtered.map((season) => (
                  <TableRow key={season.seasonId} className="border-border hover:bg-muted/30">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => setLocation(`/episodes?tvShowId=${season.tvShowId}&season=${season.season}`)}
